<?php
/**
 * Snapshot: chụp/khôi phục trạng thái plugin vào CPT odo_snapshot.
 */
...
class odo_Snapshot {
    ...
    public static function create_snapshot($action_name = 'Manual Snapshot') {
        ...
        $post_id = wp_insert_post([
            'post_type'    => 'odo_snapshot',
            'post_title'   => 'Audit: ' . $action_name . ' [' . date('Y-m-d H:i') . ']',
            'post_content' => wp_json_encode($data, JSON_UNESCAPED_UNICODE),
            'post_status'  => 'publish',
        ]);

        // Ghi log vào odo_Logger ngay khi tạo snapshot
        // odo_Logger::log('Snapshot', "Đã tạo snapshot bởi {$data['created_by']['username']} ({$data['created_by']['role']})");
    ...
    public static function restore_snapshot($snapshot_id) {
        ...
        if (!$post || $post->post_type !== 'odo_snapshot') { return false; }
