...
class TWC_Admin_UI {
    public static function init() {
        ...
        add_action('wp_head', function() {
            $header_color = get_option('twc_website_header_color', '#222222');
            if (!empty($header_color)) {
                echo '<style>
                header.site-header, .site-header, .header, .main-header {
                    background-color: ' . esc_html($header_color) . ' !important;
                }
                </style>';
            }
        });
